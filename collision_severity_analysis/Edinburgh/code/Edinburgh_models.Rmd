...
# Note: these tests justify the designation of pre- and post-20mph Edinburgh & Glasgow scenarios

# Also note: Glasgow data and models must be loaded (see Glasgow_models.Rmd)
...
# LRTT I: Model 1 (ED_POST parameters with ED_PRE data) versus Model 2 (ED_PRE parameters with ED_PRE data)

# ****NOTE: is this still applicable given that ED PRE is ordered and ED POST is binary?**** AND some variables (noteably: 'trans' and 'covid' not available in ED_PRE data)



# LRTT I: Model 1 (ED_POST parameters with ED_PRE data) versus Model 2 (ED_PRE parameters with ED_PRE data)
model_1 <- Rchoice(ksi ~ trans + first_point_of_impact_veh1_no_impact + 
                           age_band_of_casualty_cas1_over_65 + 
                           casualty_type_cas1_cyclist + 
                           casualty_type_cas1_motorcylces + 
                           light_conditions_dark_lit + 
                           ...
                           pedestrian_location_cas1_footway_verge +
                           first_point_of_impact_veh1_back + 
                           road_type_single_c, data=ED_SV_PRE, family=binomial("probit"),
                           ranp=c(road_type_single_c="n"), R=500)

model_2 <- RP_ED_SV_PRE

# LRTT I
LRTI <- lrtest(model_1, model_2)
...
# LRTT II: Model 1 (ED_PRE parameters with ED_POST data) versus Model 2 (ED_POST parameters with ED_POST data)
model_1 <- Rchoice(accident_severity ~ casualty_type_cas1_pedestrian + 
                          casualty_type_cas1_cyclist + 
                          age_band_of_casualty_cas1_over_65 + 
                          casualty_type_cas1_motorcylces + 
                          pedestrian_location_cas1_footway_verge + 
                          ...
                          age_band_of_casualty_cas1_11_20 + 
                          vehicle_leaving_carriageway_veh1_nearside + 
                          road_surface_conditions_wet, data=ED_SV_POST, family=ordinal("probit"),
                          ranp=c(vehicle_leaving_carriageway_veh1_nearside="n", # specify random parameters (ranp); n = normal distribution
                          road_surface_conditions_wet="n"), 
                          R=500) # R = number of Halton draws - 500 required for final model

model_2 <- RPBP_ED_SV_POST

# LRTT I
LRTII <- lrtest(model_1, model_2)

...
#### This notebook describes the modelling process for the Edinburgh models in the manuscript: 'Twelve Years of Evidence: Modelling the Injury Severity of Single-Vehicle Collisions pre- (SECTION 1) and post-20mph (SECTION 2) Implementation in Edinburgh and Glasgow' 
...
1.3  Correlation matrix for potential independent variables in `ED_SV_PRE` models (to control for multicollinearity).
...
1.6 Specification of more complex models (e.g., Random Parameters Ordered Probit (RPOP) and Random Parameters Ordered Probit models with Heterogeneity in the Means (RPOPHM)) using the `Rchoice` package for `ED_SV_PRE` data (if applicable).
...
# Load the Edinburgh pre-20mph (ED_SV_PRE) data; note: add your file pathname within the square brackets (i.e., the location of the 'collision_severity_analysis' folder on your PC) 
ED_SV_PRE <- read.csv("/Users/psxts6/Desktop/collision_severity_analysis/model_data/Edinburgh/ED_SV_PRE20_model_data.csv", header=TRUE, sep = ",") 
...
# Hence, do not include 'sex_of_driver_veh1_unknown' and 'casualty_type_taxi' have insufficient indicator groups to be included in the model as potential IVs; Note: ignore "accident_severity" - this is the dependent variable and does not have a binary outcome, hence, irrelevant to this chunk
...
# Some potential independent variables, especially those related to age of the casualty/driver, should be aggregated to more intuitive intervals, as follows
...
# To control for multicollinearity between potential independent variables, those with pairwise correlation of 0.3 (or -0.3) were considered to highly correlated to be included in the same model
...
# For the modelling stage, 'accident_severity' must be recoded as a factor; further, the order of 'accident_severity' must be inverted so that 1=slight, 2=serious and 3=fatal
...
# The stepwise regression (forward selection) is used to select the starting variables in the fixed parameters ordered probit (FPOP) framework
...
# Forward selection algorithm (iterative process based on biggest reduction in AIC per potential IV added to model formula)
...
## 1.6 continued: test for heterogenity in the means of random paramter variables (i.e., RPOPHM trials)
```{r}
...
# Load the Edinburgh pre-20mph (ED_SV_POST) data; note: add your file pathname within the square brackets (i.e., the location of the 'collision_severity_analysis' folder on your PC) 
ED_SV_POST <- read.csv("/Users/psxts6/Desktop/collision_severity_analysis/model_data/Edinburgh/ED_SV_POST20_model_data.csv", header=TRUE, sep = ",") 

ED_SV_POST <- na.omit(ED_SV_POST) # omit NAs (sample reduced from 1413 to 1199 - better than POSTvious; fixed error with ONS district as well as missing 2018 data; deleted 'pedestrian_movement" variable as too many NAs)
...
RPBP_ED_SV_POST <- Rchoice(ksi ~ trans + 
                           ...
                           casualty_type_cas1_motorcylces + 
                           light_conditions_dark_lit + 
                           covid + vehicle_manoeuvre_veh1_going_ahead_other + 
                           pedestrian_location_cas1_footway_verge +
                           first_point_of_impact_veh1_back + 
                           road_type_single_c, data=ED_SV_POST, family=binomial("probit"),
...
## 2.6 continued: re-estimate FPBP with RP variables (for purposes of fair comparison of FPOP and RPOP in model evaluation)
```{r}
...
## 2.6 continued: test for heterogenity in the means of random paramter variables (i.e., RPBPHM trials)
```{r}

# Random Parameters Ordered Probit with Heterogeneity in the Means of RPs (RPBPHM) trial

# No HM in RP variables produced

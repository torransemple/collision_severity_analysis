...
#### This notebook describes the modelling process for the Glasgow models in the manuscript: 'Twelve Years of Evidence: Modelling the Injury Severity of Single-Vehicle Collisions pre- (SECTION 1) and post-20mph (SECTION 2) Implementation in Edinburgh and Glasgow' 
...
1.3  Correlation matrix for potential independent variables in `GLA_SV_PRE` models (to control for multicollinearity).
1.4  Recoding, reordering & examining the distribution of the `GLA_SV_PRE` dependent variable (`accident_severity`).
1.  Stepwise regression for `GLA_SV_PRE` using the `MASS` package: identifying Fixed Parameters Ordered Probit (FPOP) model starting variables.
6.  Specification of more complex models (e.g., Random Parameters Ordered Probit (RPOP) and Random Parameters Ordered Probit models with Heterogeneity in the Means (RPOPHM)) using the `Rchoice` package for `GLA_SV_PRE` data (if applicable).
7.  Model evaluation.
...
# Load the Glasgow pre-20mph (GLA_SV_PRE) data; note: add your file pathname within the square brackets (i.e., the location of the 'collision_severity_analysis' folder on your PC) 
GLA_SV_PRE <- read.csv("/Users/psxts6/Desktop/collision_severity_analysis/model_data/Glasgow/GLA_SV_PRE20_model_data.csv", header=TRUE, sep = ",")
...
# Load the Glasgow post-20mph (GLA_SV_POST) data; note: add your file pathname within the square brackets (i.e., the location of the 'collision_severity_analysis' folder on your PC) 
GLA_SV_POST <- read.csv("/Users/psxts6/Desktop/collision_severity_analysis/model_data/Glasgow/GLA_SV_POST20_model_data.csv", header=TRUE, sep = ",")
...
## 2.6 continued: test for heterogenity in the means of random paramter variables (i.e., RPOPHM trials)
```{r}
